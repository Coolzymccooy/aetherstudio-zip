...
export const QRConnectModal: React.FC<QRConnectModalProps> = ({ roomId, sourceId, sourceLabel, onClose }) => {
  ...
  const getMobileUrl = () => {
    ...
    params.set('mode', qrMode === 'audience' ? 'audience' : 'companion');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-aether-900 border border-aether-700 rounded-xl w-[700px] shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-bold text-white">{qrMode === 'audience' ? 'Audience Message Portal' : 'Connect Mobile Camera'}</h2>
          <p className="text-sm text-gray-400">
            {qrMode === 'audience'
              ? 'Scan this code to send messages to the Studio.'
              : 'Scan this code to link your phone wirelessly.'}
          </p>
          {sourceLabel && qrMode === 'camera' && (
          ...
          <div className="flex justify-center gap-2 mt-4">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${qrMode === 'camera' ? 'bg-aether-500 text-white shadow-lg shadow-aether-500/30' : 'bg-white/5 text-gray-400 select-none'}`}
            >
              Camera Mode
            </button>
            <button
              onClick={() => setQrMode('audience')}
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${qrMode === 'audience' ? 'bg-aether-accent text-white shadow-lg shadow-aether-accent/30' : 'bg-white/5 text-gray-400 select-none'}`}
            >
              Audience Mode
            </button>
          </div>
              ...
              <div className="w-full space-y-2">
                ...
                <div className="flex gap-2 pt-2">
                  <a
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded border text-xs font-medium transition-colors ${!canShowQr
                  ...
                  <button
                    ...
                    disabled={!canShowQr}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded border text-xs font-bold transition-colors ${!canShowQr
                        ? 'bg-gray-800 text-gray-500 border-gray-700 cursor-not-allowed'
